...
class FVSwiftypeClient {
	...
	private function request($method, $path, $params = array(), $data = array()) {
		...
		$response = curl_exec($request);//echo "<!--ST response ".var_export($response,true)."-->\n";
		$error = curl_error($request);//echo "<!--ST error ".var_export($error,true)."-->\n";

		if ($error) {
	  		throw new \Exception("Sending message failed. Error: ". $error);
		}

		$http_status = (int)curl_getinfo($request,CURLINFO_HTTP_CODE);//echo "<!--ST status ".var_export($http_status,true)."-->\n";
		curl_close($request);
